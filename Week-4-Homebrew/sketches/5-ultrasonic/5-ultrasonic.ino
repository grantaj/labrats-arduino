...
const uint32_t pingInterval = 100; // ms
...
void loop() {
  ...
  // ----- HANDLE HC-SR04 DISTANCE SENSOR AS CC MESSAGE -----
  if (millis() - lastPingTime > pingInterval) {
    lastPingTime = millis();
    int distance = sonar.ping_cm();
    if (distance == 0) distance = MAX_DISTANCE;
    byte ccValue = map(distance, 0, MAX_DISTANCE, 127, 0);
    MIDI.sendControlChange(DIST_CC, ccValue, CHANNEL);
  }
}
